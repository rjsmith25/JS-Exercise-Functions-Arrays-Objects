...
function getOlderCars(cars, year) {
  let cars_with_desired_year = [];
  for (let i = 0; i < cars.length; i++) {
    if (cars[i].car_year <= year) {
      cars_with_desired_year.push(cars[i]);
    }
  }
  return cars_with_desired_year;
}
...
function carMaker(odometer) {
  return {
    ...
    drive: function(distance) {
      let updatedOdometer = this.odometer + distance;
      this.odometer = updatedOdometer;
